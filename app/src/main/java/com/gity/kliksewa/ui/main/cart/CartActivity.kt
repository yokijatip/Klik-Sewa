    ...
    class CartActivity : AppCompatActivity(), TransactionFinishedCallback {
        ...
        private fun initMidtransSdk() {
            val sdkUIFlowBuilder: SdkUIFlowBuilder = SdkUIFlowBuilder.init()
                ...
                .setMerchantBaseUrl("https://klik-sewa-merchant-server.vercel.app/")
                .enableLog(true)
                .setColorTheme(
                    CustomColorTheme(
                        "#2196F3",
        ...
        fun goToPayment() {
            ...
            lifecycleScope.launch {
                viewModel.cartItems.value.let { resource ->
                    if (resource is Resource.Success) {
                        resource.data?.forEach { cartItem ->
                            itemDetails.add(
                                ItemDetails(
                                    cartItem.productId,
                ...
                // Tambahkan shipping fee
                itemDetails.add(
                    ItemDetails(
                        "SHIPPING_FEE",
                        SHIPPING_FEE.toDouble(),
                        1,
                        "Biaya Pengiriman"
                    )
                )
...
//            create rand number every time this function hit
            val random = (1..1000).random()

            return "$prefix-$uuid-$formattedDate-$random"
        }
            ...
            if (result.response != null) {
                when (result.status) {
                    TransactionResult.STATUS_SUCCESS -> {
                        Toast.makeText(this, "Success: ${result.response.transactionId}", Toast.LENGTH_LONG).show()
                    }
                    TransactionResult.STATUS_PENDING -> {
                        // Anggap pending sebagai cancel kalau user keluar
                        Toast.makeText(this, "Transaksi belum dibayar (anggap dibatalkan)", Toast.LENGTH_LONG).show()
                        // TODO: kalau perlu, hapus transaksi dari backend atau tandai "dibatalkan"
                    }
